...
### render()로 graph 시각화하기

`course_graph.render()`는 graph 구조를 Mermaid `stateDiagram-v2` 텍스트로 돌려준다. 예제를 실행하면 다음이 출력된다.
...
이 텍스트를 Markdown의 mermaid 코드 블록이나 Mermaid Live Editor에 붙이면 그림으로 볼 수 있다.
...
"workflow를 함수 3개로만 만들면 구조가 머릿속에만 남아요. graph로 만들면 `render()`로 다이어그램을 뽑아 문서나 PR에 붙일 수 있죠. node가 늘수록 이 그림이 디버깅과 리뷰에 도움이 됩니다."
