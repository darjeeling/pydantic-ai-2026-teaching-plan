...
한 수강생은 `COURSE_MODEL=openai:gpt-5.5`로 예제가 바로 돈다. 옆자리 수강생은 같은 repo를 clone했는데 `ModuleNotFoundError`가 난다. 또 다른 수강생은 OpenRouter key를 넣었지만 tool calling이 안 되고, Bedrock을 쓰는 수강생은 Python 에러가 아니라 `AccessDeniedException`을 본다. Google Vertex를 쓰는 수강생은 모델 이름은 맞는데 project/location 설정이 없어서 막힌다.
...
agent = Agent("google:gemini-3.1-pro-preview")
```
...
export COURSE_MODEL=google-cloud:gemini-3.1-pro-preview
```
...
agent = Agent("google-cloud:gemini-3.1-pro-preview")
```
