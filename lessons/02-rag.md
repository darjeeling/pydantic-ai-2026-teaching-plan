...
이 회차의 목표는 수강생이 RAG를 "검색을 붙인 챗봇" 정도로 이해하는 데서 멈추지 않고, 문서 ingestion, chunking, embedding model 선택, vector search, answer policy, 데이터 품질이 모두 답변 품질에 영향을 준다는 점을 이해하게 하는 것이다.
...
운영팀이 새 교육 공지를 Markdown으로 추가했다. 챗봇은 "최신 수업 일정"을 묻는 질문에 여전히 예전 일정을 답한다. 개발자는 "RAG를 붙였으니 문서를 보고 답했을 것"이라고 생각하지만, trace를 보면 검색된 chunk는 예전 공지이고 새 문서는 색인되지 않았다. 다른 질문에서는 새 문서가 검색되지만, chunk가 너무 길어서 모델이 핵심 날짜를 놓친다. 또 다른 질문에서는 검색 결과가 맞는데 답변 정책이 약해서 모델이 출처 없는 추론을 섞는다.
...
따라서 이 회차의 핵심 질문은 "vector search 코드를 어떻게 짜는가"가 아니라 "답이 틀렸을 때 어느 단계가 틀렸는지 어떻게 좁히는가"이다. 수강생은 RAG를 모델 기능이 아니라 데이터 파이프라인과 답변 정책의 결합으로 보아야 한다.

강사가 던질 질문:

- "검색 결과 1등이 틀렸나요, 아니면 검색 결과는 맞는데 모델이 잘못 썼나요?"
...
Docker가 어려운 환경이면 강사는 `schema.sql`, `load_docs.py`, `rag_agent.py`를 코드 중심으로 설명하고, SQL 결과는 미리 캡처해서 보여준다.

OpenAI embedding API를 쓰기 어려운 환경이면 Ollama 또는 SentenceTransformers 기반 로컬 embedding으로 대체할 수 있다. 이 경우에도 답변 생성 agent는 기본적으로 `COURSE_MODEL`을 사용하므로, "embedding만 로컬"인지 "답변 모델까지 로컬"인지 구분해서 설명한다. 이 수업의 예제는 embedding provider만 바꿔 끼우는 구조다.
...
"1회차에서 agent는 작은 lesson catalog를 tool로 읽었습니다. catalog가 5개면 괜찮습니다. 그런데 회사의 기술 문서가 5천 개라면 어떻게 할까요? 모든 문서를 prompt에 넣을 수 없습니다. 모든 문서를 tool 결과로 반환할 수도 없습니다. RAG는 이 문제를 '먼저 검색하고, 필요한 문맥만 넣자'로 푸는 패턴입니다."
...
chunk는 검색과 답변의 기본 단위다. chunk가 너무 크면 검색은 맞아도 답변에 불필요한 내용이 많이 들어간다. chunk가 너무 작으면 필요한 맥락이 끊긴다.
...
embedding은 text를 숫자 vector로 바꾼 것이다. 의미가 비슷한 text는 vector 공간에서 가까워지도록 학습된 모델을 사용한다.

초보자용 설명:

"embedding은 텍스트의 의미 좌표입니다. 'tool 사용법'과 'function calling'은 단어가 달라도 의미가 가까울 수 있습니다. vector search는 이런 의미적 가까움을 이용합니다."
...
RAG의 품질은 모델 이름보다 데이터 상태에 더 크게 흔들릴 때가 많다. 좋은 모델을 써도 검색되는 문서가 오래됐거나, 중복됐거나, source가 없거나, chunk가 애매하면 답변은 나빠진다.

강사가 말할 핵심:

"RAG는 prompt engineering 문제가 아니라 data engineering 문제이기도 합니다. 좋은 답변을 만들려면 좋은 검색 대상이 있어야 하고, 좋은 검색 대상은 문서 구조와 metadata에서 시작합니다."
...
Apple Silicon이 아니면 `SENTENCE_TRANSFORMERS_DEVICE`를 비우거나 `cpu`, `cuda` 등 로컬 환경에 맞게 사용한다. SentenceTransformers는 모델 이름 대신 로컬 디렉터리 경로도 받을 수 있으므로, 사내망에서 미리 내려받은 모델을 쓰는 방식도 가능하다.

### pgvector

`examples/02_rag/schema.sql`의 핵심:

```sql
embedding vector(1536) NOT NULL
```

기본 예제의 `text-embedding-3-small`은 1536 dimension vector를 반환한다. 다른 embedding model을 쓰면 `vector(1536)` 부분이 달라져야 한다. 현재 `load_docs.py`는 선택한 provider의 dimension에 맞춰 schema를 동적으로 만든다. `schema.sql`은 기본 OpenAI 설정을 설명하기 위한 읽기 자료로 유지한다.
...
Vector RAG는 의미적으로 가까운 chunk를 찾는 데 강하다. 하지만 질문이 entity와 relation을 따라가야 하는 경우에는 vector search만으로 부족할 수 있다.
...
이 질문은 "장애 원인"과 가까운 문서 하나만 찾는 문제가 아니다. 프로젝트, 배포, 담당자, 장애, 조치의 관계를 따라가야 한다. 이런 경우 GraphRAG나 Knowledge Graph가 도움이 될 수 있다.
...
"처음부터 GraphRAG로 시작하지 않습니다. 기본 Vector RAG가 어떤 질문에서 실패하는지 관찰한 뒤, 관계를 저장해야 할 이유가 생겼을 때 GraphRAG를 검토합니다."
...
3. `id text PRIMARY KEY`가 재적재 중복을 막는다는 점을 설명한다.
4. HNSW index는 검색 성능을 위한 것이라고 설명하되, 수식 설명에 오래 머물지 않는다.
5. embedding provider를 바꾸면 `vector(1536)`도 바뀌어야 한다는 점을 강조한다.

말할 내용:

"RAG는 LLM 기능처럼 보이지만 실제로는 데이터 모델링 문제이기도 합니다. source와 title을 저장하지 않으면 답변에 근거를 붙일 수 없습니다. id가 안정적이지 않으면 같은 문서를 계속 중복 저장하게 됩니다."
...
5. `build_embedder()`가 OpenAI, Ollama, SentenceTransformers 중 하나를 만든다는 점을 설명한다.
...
1. `Deps`가 OpenAI client와 asyncpg pool을 가진다는 점을 설명한다.
...
5. `Deps`가 OpenAI client 대신 provider-agnostic `embedder`를 가진다는 점을 설명한다.
6. 질문을 2개 실행한다.
...
"문서를 추가했는데 답변이 나아졌습니다. 모델을 fine-tune하지 않았습니다. 검색 가능한 지식 저장소를 갱신했을 뿐입니다. 이것이 RAG의 운영상 장점입니다."
...
"RAG 운영에서 문서 품질 관리는 부가 작업이 아닙니다. 잘못된 데이터는 잘못된 답변으로 바로 이어집니다."

## 실습 3: 로컬 Embedding으로 전환

목표: 답변 생성 모델과 embedding 모델이 독립적으로 교체 가능하다는 점을 경험한다.
...
"오늘 RAG agent는 검색 tool을 한 번 호출했습니다. 실제 agent는 여러 문서를 검색하고, 여러 API를 호출하고, 결과를 필터링해야 합니다. tool call이 많아질수록 모델 왕복이 늘고 비용이 커집니다. 3회차에서는 모델이 여러 tool 호출을 Python 코드로 묶어 실행하게 하는 CodeMode와, 그 코드를 안전하게 제한하는 Monty를 다룹니다."
