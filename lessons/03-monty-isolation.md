...
이 회차의 목표는 수강생이 "LLM이 코드를 쓰게 하면 강력하지만, host에서 그대로 실행하면 안 된다"는 감각을 갖게 하는 것이다. 동시에 CodeMode를 멋진 기능이 아니라 실용적 패턴으로 받아들이게 한다. 여러 tool 호출을 줄이고, tool 결과를 로컬에서 계산으로 처리하기 위한 도구라는 자리매김이다.
