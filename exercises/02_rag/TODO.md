...
1. `examples/02_rag/sample_docs`에 `dbos.md`를 추가한다.
2. DBOS durable execution의 핵심 문장을 3~5개 적는다.
3. `uv run python examples/02_rag/load_docs.py`로 다시 적재한다.
4. `rag_agent.py`에 질문한다.
...
- 검색 결과가 없을 때 agent가 추측하지 않게 instructions를 더 강하게 만든다.
- `LIMIT 5`를 환경변수로 바꾼다.

## Data-Centric RAG 과제

1. `sample_docs`에 기존 문서와 비슷한 주제의 오래된 문서를 하나 추가한다.
2. 문서에 "이 문서는 2025년 기준이다"처럼 오래된 version 신호를 넣는다.
3. 다시 적재하고 같은 질문을 실행한다.
4. 오래된 문서가 검색 결과에 섞이는지 확인한다.
5. 실패 유형을 하나로 분류한다.
...
- Vector RAG보다 GraphRAG가 더 맞을 것 같은 질문을 2개 적는다.
- 그 질문에 필요한 entity와 relation을 적는다.
- 이번 수업에서는 왜 GraphRAG를 구현하지 않고 개념만 다루는지 설명한다.

## 로컬 Embedding 과제

OpenAI embedding 대신 로컬 embedding provider를 하나 사용해 본다.
...
로컬 embedding으로 재적재한 뒤 같은 질문을 실행하고, OpenAI embedding 결과와 retrieval 품질을 비교한다.
...
- GraphRAG가 필요한 상황과 기본 Vector RAG가 먼저인 이유를 설명할 수 있다.
