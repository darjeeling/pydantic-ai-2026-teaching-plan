...
1. Windows 수강생은 WSL 2에서 repo를 연다.
...
- repo 경로가 `/mnt/c/...`가 아니라 `~/src/...`다.
- VS Code 왼쪽 아래가 `WSL: Ubuntu`처럼 표시된다.
...
3. AI assistant는 사용하지 않는다.
   - GitHub Copilot/Copilot Chat을 끈다.
   - Cursor/Cline류 코드 생성 기능을 쓰지 않는다.
   - 자동완성은 ty language server만 사용한다.
4. `uv sync`를 실행한다.
...
5. VS Code에서 `examples/00_providers/provider_smoke_test.py`를 열고 `print_hint()` 안에서 `hint.` 자동완성을 확인한다.
6. `ProviderHint(...)`에 hover해서 field와 signature help를 확인한다.
7. Command Palette에서 `Tasks: Run Task`를 열고 `ty: check`를 실행한다.
8. 터미널에서도 같은 검사를 실행한다.
...
10. 조별로 하나의 provider를 고른다.
   - OpenRouter
   - AWS Bedrock
   - Google AI Studio
   - Google Cloud Vertex
11. 선택한 provider의 credential과 `COURSE_MODEL` 예시를 `.env`에 적는다.
12. dry-run을 실행한다.
...
15. Logfire를 사용할 수 있으면 agent run trace를 확인한다.
...
- 사용한 model string
- 필요한 환경 변수
- 필요한 capability: structured output, tool calling, image/document input, streaming 중 무엇이 필요한가
- 데이터 경로/region/logging retention에서 확인한 점
- `uv run ty check` 결과
- VS Code ty 자동완성 확인 여부
- Windows라면 WSL remote 여부와 repo 경로
- dry-run 결과
- 실제 호출 성공/실패 여부
...
- provider 이름과 model capability가 다를 수 있음을 설명할 수 있다.
