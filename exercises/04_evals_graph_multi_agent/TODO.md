...
3. source marker가 빠지면 실패하는지 일부러 확인한다.

## 과제 B: RAG / Tool-use Eval

1. `examples/04_evals/rag_tool_use_eval.py`를 실행한다.
2. `rag_question_retrieves_lesson_02` case에서 기대 source가 무엇인지 확인한다.
3. `dangerous_write_requires_approval` case가 왜 `delete_database`가 아니라 `request_human_approval`을 기대하는지 적는다.
4. 일반 설명 질문 case를 하나 추가하고 `tool_calls=()`를 기대값으로 둔다.
5. 일부러 task가 `retrieve_course_docs`를 호출하게 바꿔 실패를 확인한다.
6. `ToolCallPolicy`의 forbidden tool 목록에 `charge_credit_card`를 추가한다.

## 과제 C: Graph 변경

1. `Review` node에서 답변 길이가 너무 짧으면 reject하게 한다.
2. `course_graph.render()` 출력의 Mermaid diagram을 슬라이드에 붙인다.

## 과제 D: 모델 전환 Eval 설계

1. `examples/04_evals/compare_models.py`를 읽는다.
2. `EVAL_MODELS`에 비교할 모델 2개를 넣는 명령을 작성한다.
3. 모델 전환 승인 기준을 3개 적는다.
...
2. PR마다 실행되는 job과 수동 실행되는 job을 구분한다.
...
3. usage limit을 낮춰 실패 케이스를 관찰한다.
...
5. reviewer agent로 대체해도 되는 검토와 사람이 승인해야 하는 검토를 구분한다.
...
- 모델 전환 시 pass rate, token usage, critical case 실패 여부를 비교해야 함을 설명할 수 있다.
