...
1. `httpx_raw_api_log.py`를 실행하고 raw HTTP request/response가 파일에 남는 것을 확인한다.
2. raw 로그에 prompt와 response body가 남는다는 점을 확인하고, 왜 공유하면 안 되는지 메모한다.
3. `tool_agent.py`에서 `list_lesson_ids` 대신 lesson id와 title을 함께 반환하는 `list_lessons` tool을 만든다.
4. 존재하지 않는 lesson id를 요청하면 agent가 임의로 답하지 않고 "교안에 없음"이라고 답하게 한다.
...
- 출력 JSON에 관련 lesson id가 포함된다.
- `confidence`가 0과 1 사이 값이다.
- `next_action`이 수강생 행동으로 읽힌다.
